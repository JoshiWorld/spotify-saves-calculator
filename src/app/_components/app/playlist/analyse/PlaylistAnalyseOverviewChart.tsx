...
export function PlaylistAnalyseOverviewChart() {
    const isMobile = useIsMobile();
    const [timeRange, setTimeRange] = React.useState("90d");
    const { data } = api.playlistAnalyse.getOverviewChart.useQuery();
    ...
    const maxVal = Math.round(Math.max(...filteredData.map((item) => item.follows)) * 1.1);
...
export function PlaylistAnalyseOverviewChartView({ id, statsRange }: { id: string; statsRange: string }) {
    // const isMobile = useIsMobile();
    const { data } = api.playlistAnalyse.getDailyFollows.useQuery({
        playlistId: id,
    ...
    const maxVal = Math.round(Math.max(...data.map((item) => item.follows)) * 1.1);
